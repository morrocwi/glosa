...
- RET-Check v0.2 (2026-09-08): Toledo registrar assigned codes to the RET equations the program implements (`methodology/P19`; Toledo v1.5.0 pending, concept DOI 10.5281/zenodo.22537318) — `RET-N04 = EQ-015/H.41.v1`, `RET-N06 = weld/H.36.v1`, `RET-N09 = weld/H.37.v1`, `RET-N10 = EQ-015/H.43.v1`, `RET-N16 = A.8/M.21.v1`, `RET-N17 = A.8/M.22.v1`, `RET-N18 = A.8/M.23.v1`. `scripts/ret_check.py` version string bumped to v0.2 and every relevant docstring/report line now cites the Toledo code next to its `RET-Nxx` alias; `methodology/P21_ret_check.md`, `cli/README.md` updated to match; `cases/ret/PREREGISTRATION_v0_1.md` carries a dated "codes added; operator unchanged" note. No computed quantity, RET RISK rule, or pre-registered scenario's expected output changed; `cases/ret/SELF_AUDIT_RET_v2_0.txt` regenerated with the new report text (same HIGH verdict).

- RET-Check v0.1 (founder order 2026-09-07, `BBL-2026-09-07-223`): `methodology/P21_ret_check.md` + `cases/ret/PREREGISTRATION_v0_1.md` (pass/fail scenarios A–E declared before any code) + `scripts/ret_check.py` (stdlib-only, no network, `AI=0` at runtime) computing N_A/N_P, recursive cycles, external interruption, and a RET RISK label from `The_Recursive_Epistemic_Tunnel_GENESIS_FIRST_FULL_v2_0.md` §13–21/§25–30's own definitions; `./cli/glosa ret check <case>` verb; `tests/test_ret_check.py` (scenarios A–E, self-application, CSV/JSON input, invalid-input exit 2, stdlib-only/no-network guard); `cases/ret/SELF_AUDIT_RET_v2_0.txt` — the manuscript's own Core Epistemic Registration modeled and audited, reporting HIGH per the preregistration.

- 2026-09-07: `scripts/check_forbidden_words.sh` gains a third result class, `[QUOTED-SOURCE]` (counted and printed, never fails), for a forbidden word sitting inside a record's own verbatim Zenodo abstract as quoted into a generated `docs/library/` knowledge-graph file (`scripts/zenodo_library_kg.py`); `zenodo_library_kg.py` now stores that text under a `quoted_abstract_head` key and the generated `AIHP_LIBRARY_KG.md` renders it inside an explicit "Quoted from the record's own abstract (verbatim, not a glosa claim)" block, so a scanner or a reader can tell a quoted third-party word from a glosa-authored one by the key/label alone; regenerated `docs/library/aihp_kg.json` and `docs/library/AIHP_LIBRARY_KG.md` from live Zenodo metadata, unblocking the commit that had been discarded because one member record's own abstract (Dialogue Conversion Protocol v1.3) quotes, in its own words, that it replaced "novelty" wording with a contribution boundary. The author's own voice is unaffected — every other path stays a hard fail on any of these words, with no new allowlist entries.
...
- P20 enforcement (founder ruling 2026-09-07, BBL-2026-09-07-218): `schema/core_epistemic_structure.schema.json` (+ example); `kernel.ces_check_text`/`kernel.validate_core_epistemic_structure`/`kernel.ces_check_path`; `gate_release` hard-fails a release manifest missing or failing `core_epistemic_structure`; `scripts/check_core_epistemic_structure.py` + `glosa ces check <path...>` CLI verb check a raw Markdown/LaTeX/YAML/JSON file directly; `templates/paper/{arxiv-onecol,arxiv-twocol}/main.tex` now carry the block; `scripts/check_repo.sh` runs the same check pre-commit, scoped to `templates/paper/*/main.tex` and any file declaring `ces: required` in its first 5 lines (never a historical file that predates this ruling; `templates/knowledge/*` data-card and checklist templates are named out of scope) — see P20's own Gate section for the exact scoping sentence.
