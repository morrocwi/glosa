...
| `citation_card.schema.json` | §7.8 | Integrity Firewall's two independently-flaggable booleans. `status` uses FOUNDATION's newer six-value enum, not the broader draft enum in `templates/knowledge/citation_card.yaml`. Optional `chaining` (backward/forward, once per source) and `audit` (`VERIFIED`/`RELAYED`/`OVERREACH`, sentence-level; `VERIFIED` requires `claim_match_verified: true`) added 2026-09-25 (P26), backward compatible. |
...
| `claim_evidence_matrix_row.schema.json` | `methodology/P26_claim_evidence_theory_matrix.md` (c), 2026-09-25 | One row per claim × source use; a join over claim card, citation card and search log that holds only construct, source type, three-tier role, supports / does-not-support, rival evidence and manuscript use. Empty `rival_evidence` requires `rival_search_ref`. |
| `theory_evidence_falsifier_row.schema.json` | `methodology/P26_claim_evidence_theory_matrix.md` (f), 2026-09-25 | One row per proposition: mechanism + premises (no argument by citation), supporting / contradictory matrix rows, boundary conditions, observable implication, rivals. Falsifier text stays on the claim card; `falsifier_on_card: false` keeps the row at `Draft`. |
...
| `fail_citation_audit_verified_without_claim_match.json` (citation_card-shaped) | `audit.status: VERIFIED` while `claim_match_verified: false` | P26 item (e), schema-enforced (citation_card allOf), 2026-09-25 |
...
| 22 | `citations[].intake_tier` / `intake_tier_reason` / `global_south_exempt` (INTAKE-TIER-UNTIERED) | **shipped in kernel, warning-only** (`rule22w`, thin scope ratified DECISIONS.md 2026-09-05 BBL-2026-09-05-122 — a `request_tier` row with no `intake_tier_reason` is logged, never a hard block, never merged with a rejected row) | `schema/litreview_manifest.schema.json`'s additive `citations[].intake_tier`/`intake_tier_reason`/`global_south_exempt`; `kernel/glosa_kernel.py` `_intake_tier_warnings`, wired into `lit_gate` |
...
| 24 | Premature Category Stabilization (PCS), joint closure-timing + absence-of-adaptation | **shipped in kernel** (narrow scope ratified DECISIONS.md 2026-09-05 BBL-2026-09-05-122 — ERROR `rule24` only when BOTH conjuncts hold jointly; WARNING `rule24w` when only one holds; silent otherwise; a lexical heuristic, evadable by omission like rule 8/18/21; kept distinct from the unrelated clinical term "premature closure", see `methodology/data/contaminated_concept_table.json`'s PCS row) | `kernel/glosa_kernel.py` `_pcs_signals`/`_pcs_error_and_warning`, wired into `validate_claim_card`; fail fixture `schema/examples/fail/fail_rule24_pcs_joint_condition.json` |
| 25 | `search_log.candidate_set_stage{candidates_generated, candidates_collapsed, routing_note}` (DISCOVERY-STAGE-ABSENT) | **shipped in kernel, warning-only** (`rule25w`, optional-S14-stage scope ratified DECISIONS.md 2026-09-05 BBL-2026-09-05-122 "optional S14 stage" — narrower than the patch §23 draft's heavier `discovery_routing`/k_epi-gated mechanism, which stays pending-founder, see the schema `$comment`; fires only when `search_log.search_mode` is SYSTEMATIC_REVIEW and no `candidate_set_stage` is recorded on either the search_log or the manifest, and only when `search_log` is actually supplied to `lit_gate` — an honest silence otherwise, never a fabricated pass) | `schema/search_log.schema.json`'s additive `candidate_set_stage` field; `kernel/glosa_kernel.py` `_discovery_stage_warning`, wired into `lit_gate(manifest, search_log=None)` |
...
| 32 | NC-78 Potential ≠ Exercised ≠ Observed: a capability/potential-worded claim must name which readout it is about via `five_questions.seen.capability_readout` (`potential_witnessed` / `exercised` / `observed`) | **shipped in kernel, warning-only** (`rule32(NC78-READOUT-UNNAMED)`; lexical heuristic; founder ruling BBL-2026-09-05-130/132; paper *Potential as a Readout*, Society/Justice/Peace series) | `kernel/glosa_kernel.py` `_rule32_capability_readout_warning`; `tests/test_kernel.py` `NC78NC79WarningTest` |
| 33 | NC-79 Diagnosis of compression ≠ Attribution of responsibility: responsibility wording on a structural-compression claim needs `scope.responsibility_attribution.chooser` (Causal Ethics Axiom II / CE-08); diagnosis alone is always allowed | **shipped in kernel, warning-only** (`rule33(NC79-ATTRIBUTION-WITHOUT-CHOOSER)`) | `kernel/glosa_kernel.py` `_rule33_attribution_warning`; same test class |

Every rule 18–28 row above traces to `design/FOUNDATION_v0.6_PATCH.md`. Rules 22/24/25 were the
three `pending-founder` DAG nodes DECISIONS.md 2026-09-05 (BBL-2026-09-05-122) ratified at a
narrower/thinner scope than the patch's own draft text — each row above states the exact ratified
scope and, where it differs from the heavier patch-draft mechanism, names what remains
pending-founder (e.g. rule 25's `discovery_routing`/k_epi-gated mechanism itself). Rules 29/30/31
trace to `design/SESSION_ARCH_v0.4_SPEC.md` §10.2/§10.3/§10.4 instead.

D-SCOPE-CONTEXT (§14, `foundation.s5-scope-boundary-per-instance`, DECISIONS.md 2026-09-05
BBL-2026-09-05-122: per-instance clause + context check, no boilerplate) is a disclaimer-catalogue
row, not a numbered §3.3 kernel rule — WARNING-only, fires when a claim card reads health/legal
domain (the same scan `_DOMAIN_SAFETY_KEYWORDS` already backs for D-NOT-DIAGNOSTIC) and carries no
`scope.per_instance_scope_clause`. See `methodology/data/disclaimer_catalogue.json`'s
`D-SCOPE-CONTEXT` row, `claim_card.schema.json`'s additive `scope.per_instance_scope_clause`
field, and `kernel/glosa_kernel.py`'s `_scope_context_warning`.
