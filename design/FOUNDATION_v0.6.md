# Rigour Without Infrastructure — Foundation v0.6 (repo: glosa)

> **Tier: Dr** (sixth-pass synthesis — merges every READY section of
> `design/FOUNDATION_v0.6_PATCH.md` into v0.5 mechanically, per this task's own scope
> (K6-foundation). Every `pending-founder` section from the patch is inserted **as a clearly
> marked "PENDING FOUNDER DECISION — recommended text follows" block**, never silently treated as
> ratified. **None of the schema/kernel/template files these sections describe were edited by
> this pass** — same documentation-only scope as v0.5 (this task's own instruction: `design/`
> only). Every open item from the patch is carried forward, not silently closed. K-state
> unchanged: **K0** (no independent check has run on this synthesis pass itself, `NC-34`). Not
> itself independently re-checked — this document must still pass a fresh `PUB-ADVERSARIAL-REVIEW`
> / MIMCG round before any of it is treated as settled. readout-not-truth applies to every line
> below: this is a *proposal*, not a ratified specification.

## 0. Changelog v0.5 → v0.6

- **New rule table entries 15–28 in §3.3** — rules 15–17 (already shipped in
  `kernel/glosa_kernel.py` but previously undocumented in FOUNDATION's numbered prose: responsibility
  arrow / ownership, empirical-extension warning, source-first citation) folded into the numbered
  list; rules 18–28 (this pass's own new specification: injected-infinity/zero taxonomy, Fail-Able
  Gate Law, novelty-word rejection, layer-mismatch flag, intake-tier untiered, verdict-class
  unlisted, PCS joint-condition, discovery-candidate ungated, composite-quote, hidden-AI-fill,
  inflated-bearing) added per `design/FOUNDATION_v0.6_PATCH.md` §1–§23, K-C1–K-C3.
- **New schema fields (additive, schema_version target `0.7.0` — see §3.2's own version-history
  note):** `comparison`, `evidence_strength`, `verdict_class`, `gate_fail_taxonomy`,
  `gate_construction_status`, `five_questions.seen.ai_assisted_fields`.
- **Non-collapse table:** Appendix A gains Family J (`NC-74`..`NC-76`), "73 rows across eight
  families" → "76 rows across nine families."
- **New named-source citation-only additions** (no wording change to any base doctrine text): §1.0
  Pillars cross-tradition citation set, §2.1b mother-equation framing, §3.1 E-A-D crosswalk source,
  §3.2b/§7 Possession-Constitution Collapse source, §4.2 I0–I5 named source, §4.4 K-state named
  source, §2.3 Blackbox Log named-origin citation, §12 Agrippa's trilemma applied to the tier
  system.
...
- **Companion:** `design/REPO_SPEC_v0.6.md` (file/dir diffs); `design/CURRENT_SPEC.txt` now names
  this pair as canonical.
- **Tooling gaps this run found** (repeated here, not only in `CHANGELOG.md`, per this document's
  own honest-edges discipline, §12): (1) rule 18's original draft mis-scanned for an "I3" family
  match against kernel rule 8's unrelated `EXTERNAL_VALIDATION_PROPOSED` scan before the review
  response corrected it to a standalone new scan (I3-route-fix, i.e. the injected-infinity/zero
  detector is its own rule, not a layer on rule 8); (2) `scripts/check_leak.sh` and
  `scripts/check_forbidden_words.sh` both exit non-zero against the pre-existing corpus (162/34
  un-allowlisted hits respectively, per `REPO_SPEC_v0.5.md` §9) — this pass's own leak-scan rewrite
  is **not** included in this synthesis (design/-only scope; flagged as a tooling gap for the
  scripts-owning fixer, not silently treated as fixed); (3) a spot-check of this synthesis pass's
  own mechanical patch-application (does every "exact text to insert" block actually appear,
  verbatim, at its named target) has not been independently run — logged here as the same
  same-model/single-pass gap `design/FOUNDATION_v0.6_PATCH.md`'s own K0/`NC-34` status names for
  itself, now inherited by this merged document.

---

> **Fifth-pass header, carried forward below for lineage (v0.5's own front matter):**

> **Tier: Dr** (fifth-pass synthesis — merges `design/FOUNDATION_v0.5_PATCH.md` into v0.4
> mechanically, per `ARCHITECTURE_REVIEW_v1.md` MUST-1 and founder ratification (§7 decision 1
> there). Adds §1.0 "Pillars — ontology / epistemology / methodology" (new subsection), Appendix A
> Family I (`NC-65`..`NC-73`), `tested.evidence_relations[].channel` + kernel rules 13/14 (§3.3),
> `review_report.rpe_check` (§7.1, spec only), the AI-disclosure template (§5), and the
> `lens_used.references` extension note (§2.3) — all as documentation-layer spec text. **None of
> the schema/kernel/template files these sections describe were edited by this pass** — this pass
> is documentation-only (`ARCHITECTURE_REVIEW_v1.md` MUST-1/MUST-9 scope); every open item is
> flagged inline and repeated in `design/FOUNDATION_v0.5_PATCH.md`'s own "Open items" list, which
> stays live as this version's outstanding-implementation record. Carries forward v0.4 unchanged
> except where this document states otherwise. Not itself independently re-checked — this document
> must still pass a fresh `PUB-ADVERSARIAL-REVIEW` / MIMCG round before any of it is treated as
> settled.
> readout-not-truth applies to every line below: this is a *proposal*, not a ratified
> specification.
>
> **Fourth-pass header, carried forward below for lineage (v0.4's own front matter):**
...
> thickens *kasb* with intention/orientation) maps to the Principle of Sufficient Reason — both
> cited directly (kc-ep-030, kc-ep-034) as parallel formulations, comparison: same structural role,
> not identical content. **Excluded by the same reasoning, stated here rather than only in a
> commit message:** "Dependent Order" (a three-tiered causality account per `kc-ep-030` — this
> construct does not carry *kasb*; *kasb* belongs to Directed Selection, adopted above, not to this
> excluded construct) and the five registers of faqr as a whole are not given a structural analogue
> — their content is specific to their originating discourse in a way the other two constructs are
> not, and forcing an analogue would misrepresent rather than translate them. This is a partial
> adoption by design, not an oversight. Evidence: kc-ep-030, kc-ep-034.
...
**Cross-tradition citation set (this pass, v0.6, `design/FOUNDATION_v0.6_PATCH.md` §15).** The
readout-not-truth stance, the bounded-knower framing, "tracking truth" as a named epistemic target,
and the δ_R primitive each appear, independently arrived at, in the cited base texts and in
kc-base-001, kc-base-005, kc-base-009, kc-base-027, kc-base-032, kc-ep-014, kc-ai-019,
kc-islam-008. These are cited as **parallel, independently-arrived-at formulations** — comparison:
same underlying distinction, cited, never as external validation of glosa's own stance
(`EPIS-KNOWLEDGE-VALIDATION`) and never as priority/first-arrival framing (gate rule 6).
...
**Named source (this pass, v0.6, kc-ep-052, kc-ai-002, `design/FOUNDATION_v0.6_PATCH.md` §20).**
The I0–I5 Independence Ladder and the falsifiable-propositions framing already used in this section
are cited to "Rigour Without Infrastructure" — the ladder's existing five classes and definitions
are unchanged.
...
**Named source (this pass, v0.6, kc-ep-041, kc-aihp-017, kc-ai-011, `design/
FOUNDATION_v0.6_PATCH.md` §21).** "The Standalone Scholar" is cited as the named origin of the
K-state concept and the PRESERVE_EXACT dual-track architecture already used in this section —
...
**Reusable disclaimer patterns (new this pass, v0.6, `design/FOUNDATION_v0.6_PATCH.md` §13 — not
new `D-*` ids, reusable prose templates cited by name where used):**
...
2. *"Analytical tool, not verdict"* — used when a schema/checklist/framework produces a structured
   description, not a pass/fail judgment on the thing it describes (kc-he-015).
...
4. *Reflexive practitioner standpoint/conflict-of-interest disclosure* — used whenever the author's
   own practice, role, or position could plausibly bias the claim being made (kc-islam-016,
   kc-se-006).

These are distinct in scope from the per-instance jurisdiction clause immediately below — that
...
> even exist/apply in this jurisdiction). **Named risk, tracked not solved by this rule:**
> per-instance variation risks becoming unmanaged duplication across hubs/deployments — this rule
...
> mechanism (comparison: different, not same). See §3.3 rule 24. Evidence: kc-ep-018, kc-ep-019,
> kc-ep-020, kc-islam-011, kc-tourism-002.

**glosa AI-disclosure template (new this pass, v0.5, §1.0 Pillars) — reuse in any glosa
document's own AI-assistance disclosure** (paper front matter, README, claim card
...
> changes your assessment of this claim, name the reliability, independence, or evidential channel
> it affects (`rpe_check`, §7.1) rather than the fact of AI involvement alone — an unnamed channel
> is a residual, and this document requires residuals to be named or dropped, never left silent."
...
> fixture batch confirms otherwise. Tier of any "N flagged, zero false positives" claim about this
> check: `finite_diagnostic`, scoped to the corpus it was run against — never stated as a general
> detection rate. See §3.3 rule 21. Evidence: kc-islam-009.
...
**New this pass (v0.5, §1.0 Pillars) — `review_report.rpe_check`, spec only, not yet applied to
`schema/review_report.schema.json`:** a required block whenever `produced_by` is not a single
fixed value across the route being reviewed (i.e. whenever the reviewer could, in principle,
imagine swapping human/ai/joint with everything else held fixed) — the Residual Provenance Effect
check (Definition 4 / Principle 4, `design/S15_pillars-ontology-epistemology-methodology.md`
Pillar 2 §2.4):
...
**Companion kernel rule (spec only):** `rpe_check.residual_named` and `rpe_check.residual_dropped`
cannot both be empty/false when `credence_before_channel_fix ≠ credence_after_channel_fix` — a
hard validation error, not a disclaimer-and-proceed (same enforcement pattern as §3.3 rule 12's
`D-LENS-UNSIGNED`). This edit belongs to whichever fixer owns `schema/review_report.schema.json`
and `kernel/glosa_kernel.py` (open item, see `design/FOUNDATION_v0.5_PATCH.md` §4).
...
**Composite-quote detector (new this pass, v0.6, K-C1, `design/FOUNDATION_v0.6_PATCH.md` K-C1 —
closes a K4 sim-confirmed gap).** A citation card's `exact_passage` containing an ellipsis marker
("…", "...", or a spaced double-hyphen " -- ") splicing across a boundary is a composite quote:
text assembled from non-contiguous source material and presented as one continuous passage. This
is a hard error, not a disclaimer — a composite quote defeats `claim_match_verified` by
construction, since no single passage in the original source reads as quoted (`NC-18` Source
existence≠Claim support, applied to passage-level contiguity). Scope note: this check reads only
the existing `exact_passage` field; `page_or_locator` and `line_or_paragraph` are unchanged. See
§3.3 rule 26.

**Inflated-bearing detector (new this pass, v0.6, K-C3, `design/FOUNDATION_v0.6_PATCH.md` K-C3 —
closes the kernel-rule half of the `comparison`/`evidence_strength` schema addition, §3.2 above,
kernel-rule half).** A `tested.evidence_relations[]` entry with `bearing: SUPPORTS` is a hard error
**unless** `strength` explicitly states "context" (i.e. the entry is honestly scoped as
background/context support, not evidentiary support) when **either**: (a) `citation_ref` does not
resolve to any known `citation_card_id` in the run's citation set, or (b) the *resolved* citation
...
case the schema-only `evidence_strength` field cannot catch alone: a card can satisfy the schema
shape while its own evidence relation is either unresolvable or self-disclosed as
context/same-lineage. See §3.3 rule 28.
...
> normally suppress an AACODS trace-to-original+appraise pass (e.g. a policy brief, a
> grey-literature report, an anecdotal/expert/local-knowledge account) — the flag requests a tier
> assignment rather than rejecting the source outright. **Explicit exemption:** Global South
> anecdotal, expert, and local-knowledge evidence genres are never treated as automatically suspect
> by this flag alone; the flag names a gap in the existing checklist's coverage, not a downgrade of
> the source's standing. A flagged-and-tiered row is distinct in the manifest from a rejected row.
> See §3.3 rule 22. **Sim caveat carried forward explicitly:** any acceptance test for this
> mechanism must be run against fixtures built for the actual AACODS-layer mechanism, not against
> the existing S4 corpus (which has no such fixtures). Evidence: kc-ai-024, kc-he-018.
...
> `k_epi` (the epistemic-multiplication construct, cited kc-ep-045, kc-ep-046, kc-ep-048,
> kc-ep-049, kc-aihp-005, kc-aihp-006, kc-aihp-010, kc-aihp-020) as the gating construct. This is an
...
> already-`holds`/`adapt` cards, not the open agenda claim itself. See §3.3 rule 25.
> `litreview_manifest.yaml` gains a `discovery_routing: { used: bool, candidate_questions:
> [string], k_epi_gate_log: [{question, gated_out: bool, reason}] }` block (additive).
...
**Named source (this pass, v0.6, `design/FOUNDATION_v0.6_PATCH.md` §19 — patch target stated as
§3.2b/§7, placed here instead since rule 17's own provenance-only-channel prose actually lives in
this section, not §3.2b; flagged rather than silently relocated).** "Possession-Constitution
Collapse" and "knower fetishism" (cited: kc-ep-043, kc-ai-003, kc-ai-004, kc-aihp-013, kc-aihp-014)
are the named source for why an AI-drafted claim card is not disqualified by authorship alone, and
for the "channel over face" / provenance-only-metadata rule already implicit in the source-first
citation rule above — the rule text itself is unchanged; this adds the citation.
...
**Agrippa's trilemma, applied to the tier system itself (new this pass, v0.6, `design/
FOUNDATION_v0.6_PATCH.md` §12).** The tier ladder (`Th_coqc | finite_diagnostic | fit_calibrated |
Dr | definition | Open`) is not self-certifying — any justification chain for "why trust this tier
assignment" terminates in infinite regress, circularity, or an unjustified stopping point
(Agrippa's trilemma, cited kc-base-025, kc-base-031, kc-base-036), exactly like any other
justificatory chain. The tier system's honest response is not to claim escape from the trilemma
but to make the stopping point explicit and checkable (`independent_check`, §7.2's
maker/checker/approver separation) rather than silent — the trilemma is why
`EPIS-KNOWLEDGE-VALIDATION`'s horizontal-only stance is a design necessity, not a preference: there
is no non-circular vertical stopping point available to appeal to instead.
...
**This v0.6 pass specifically:** mechanically merges every READY section of `design/
FOUNDATION_v0.6_PATCH.md` (itself K0, single-pass, same-model per that document's own header) into
this document; six sections land as `PENDING FOUNDER DECISION` blocks, never as ratified text. No
independent check has run on this merge itself — the same K0/`NC-34` status the patch document
already carried is inherited here, not upgraded by the act of merging.
...
Reproduced from `design/S9_non-collapse-table.md`, plus three rows appended v0.4 (chair ruling
C6 — append-only, ids never reused) and nine more rows (`NC-65`..`NC-73`, Family I) appended this
pass (v0.5, §1.0 above), plus three more rows (`NC-74`..`NC-76`, Family J) appended this pass
(v0.6, `design/FOUNDATION_v0.6_PATCH.md` §16). 76 rows across nine families. Full table, columns, and "how the table is
used" mechanism preserved unabridged in the source file; the compact index below gives id, pair,
and the glosa field/gate that enforces it.
...
### Family I — Possession / Constitution (new this pass, v0.5)
...
`evidence_relation.channel` (§3.2), `rpe_check` (§7.1), `D-NO-VERTICAL-AUTHORITY`. Full rows:
`design/S9_non-collapse-table.md`.*

### Family J — Cross-tradition parallels (new this pass, v0.6)
`NC-74` Equal-distinction-count≠Equal-epistemic-standing (uncited pending a matching source — no
base/ep/ai card was found this pass that states the distinctions-count claim directly) · `NC-75`
Same-shaped formula≠Same tier (three formally identical tiers can carry different epistemic
weight; kc-base-021 is a direct match; kc-ep-021 is cited as a convergent cross-tradition
philosophical anchor, distinct in strength from kc-base-021's direct match, not as an equally
strong statement of the same specific claim) · `NC-76` θ(E) relational invariance≠θ(E) absolute
value (kc-ep-023, kc-ep-024, kc-ep-026, kc-ep-027). *Enforced by: `tier`,
`evidence_strength.class` (§3.2), §1.0 Pillars. Full rows: `design/S9_non-collapse-table.md`.*
