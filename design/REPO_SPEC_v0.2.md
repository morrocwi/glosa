# RWI Repo Spec v0.2 — directory tree, one-line purpose, owner-rule

> **ARCHIVED — superseded by `FOUNDATION_v0.5.md` / `REPO_SPEC_v0.5.md`; kept for lineage. Do not use as current spec.**


> **Tier: Dr.** Companion to `design/FOUNDATION_v0.2.md`. Updates `REPO_SPEC_v0.1.md` for the
> renames and consolidations `design/CHAIR_RULING_v1.md` binds: Blackbox Note (A1), one disclaimer
> catalogue (C2), per-project ledgers + merged view (B6), per-project kg + merged view (already
> true in v0.1, restated per B6's pattern), the 9-genre router with `venue_track`/`companion_of`
> attributes (B1), `plugins/rigour-without-infrastructure/`, `kernel/`, `cli/` (unchanged from
> v0.1 — S7's `plugins/rwi/` rename is still not adopted).
>
> **NOT YET ON DISK.** Every path below is a design-time specification. As of this synthesis pass
> (2026-09-04), nothing in this tree exists as a file or directory on disk except the design/
> reviews/sources/surveys files already present in this repo — check `git status`/`ls` before
> assuming any path below is real. Founder must still ratify repo name, license split, and TH/EN
> direction (`FOUNDATION_v0.2.md` §11) before P-A bootstrap creates any of this for real.
>
> **One-fact-one-home discipline, unchanged from v0.1:** a file in `methodology/` states a rule in
> prose. A file in `schema/` states a rule's shape. A file in `methodology/data/` states a rule's
> *content* when machine-read. A file in `kernel/`/`scripts/` enforces a rule. A file in `paper/`
> narrates a rule by citation. A file in `plugins/`/`cli/`/`mcp/` invokes a rule. No file may do
> two of these five jobs for the same rule.
...
├── LICENSE                            # MIT — scripts/, kernel/, cli/, mcp/, schema/*.json,
│                                       #   CI workflow files.
├── LICENSE-TEXT.md                    # CC BY 4.0 full text — paper/, methodology/*.md, cases/,
│                                       #   incidents/, README prose, templates/*.md.
├── LICENSES.md                        # path-glob → license map. R2/check_repo.sh walk this.
├── CLAIM_BOUNDARY.md                  # L0 — RWI's own claim ceiling about itself.
...
│                                      #   + MIXED_GENRE, chair ruling B1), venue_track,
│                                      #   companion_of.
│   ├── claim_card.schema.json         #   FOUNDATION §3.2, the canonical schema — v0.3.0. Adds
│                                      #   `shape: stub|full` (chair ruling B2), `origin_blackbox_
│                                      #   ref` (replaces origin_r0_ref + origin_dialogue_ref,
│                                      #   chair ruling A1/C8), `venue_track`, `companion_of`.
...
│                                      #   by this synthesis.
│
├── records/                           # RWI's OWN S1–S3 pipeline artifacts (self-application
...
│                                      #   this top-level directory is `rwi kg merge`'s OUTPUT,
│                                      #   never hand-edited).
│   ├── nodes.jsonl
│   └── edges.jsonl                     #   Generated FROM the per-project cards (`rwi kg merge`) —
...
│   │                                  #   ledgers, output of `rwi ledger merge`, never hand-edited.
...
├── plugins/
│   └── rigour-without-infrastructure/  #   Canonical name (unchanged from v0.1 — S7's "plugins/
│       │                              #   rwi/" rename is still not adopted).
│       ├── .claude-plugin/plugin.json
│       └── skills/
│           ├── rwi-intake/SKILL.md            → cites methodology/P2_intake.md
│           ├── rwi-claim-card/SKILL.md        → cites methodology/P3_claim_card.md
│           ├── rwi-evidence-challenge/SKILL.md → cites methodology/P5_evidence_challenge.md
│           ├── rwi-independent-check/SKILL.md  → cites methodology/P6_independent_check.md
│           ├── rwi-disclaimer-system/SKILL.md  → cites methodology/data/disclaimer_catalogue.json
│           ├── rwi-publish-gate/SKILL.md       → cites methodology/P10_publish_gate.md
│           ├── rwi-genre-router/SKILL.md       → cites methodology/P13_genre_router.md
│           ├── rwi-project-advisor/SKILL.md    → cites methodology/P14_project_advisor.md
│           ├── rwi-cite-check/SKILL.md         → cites methodology/P9_vet_external_input.md
│           │                                  #   + FOUNDATION §7.8 citation firewall
│           └── rwi-blackbox-note/SKILL.md      → cites methodology/P15_blackbox_note.md (NEW,
...
│   ├── rwi_protocol_kernel.py         #   validate_*, compute_disclaimers, route_genre,
...
│       │   test_stub_cannot_advance.py (NEW — chair ruling B2's kernel rule 9) ·
...
│
├── cli/
│   └── rwi                             #   Console-script entry point calling kernel.main.
│                                      #   Adds `rwi ledger merge` alongside `rwi kg validate|
│                                      #   merge|render` (chair ruling B6).
│
├── mcp/
│   ├── rwi_mcp_server.py               #   One tool per kernel function, incl. `rwi_ledger_merge`.
...
│                                      #   build artifacts committed; LICENSES.md glob coverage.
...
## Resolved-from table — every deviation from `REPO_SPEC_v0.1.md`, and why

| Deviation | v0.1's tree | This spec (v0.2) | Resolved per |
|---|---|---|---|
...
| `methodology/P15_blackbox_note.md` added | Not present (P0–P14 only) | New card, split out from the intake/claim-card cards it was implicit in | Request 32's mandatory-appendix rule now needs its own protocol card; `plugins/.../rwi-blackbox-note/SKILL.md` added to match |
| `records/blackbox/<id>.yaml` added | Not in v0.1's `records/` tree | New subdirectory, per project | Chair ruling A1 + B6 pattern |
| `kg/` becomes a MERGED-VIEW-ONLY directory; canonical home moves to `<project>/kg/` | v0.1: `kg/{nodes,edges}.jsonl` at repo root "+1 per project," ambiguous about which was canonical | Explicit: per-project is canonical, repo-root is `rwi kg merge`'s generated output only | Chair ruling B6 — same pattern extended from the kg (already partially per-project in v0.1) to the ledgers, made explicit for both |
...
Every path in the tree above — including every renamed/added path — is a design-time
specification only. `git status`/`ls -la` on the actual `rigour-without-infrastructure/` working
tree as of this synthesis pass shows only `design/`, `reviews/`, `sources/`, `surveys/`, and this
repo's own root `HANDOFF_*`/`PLAN_*` files; no `paper/`, `methodology/`, `schema/`, `kernel/`,
`cli/`, `mcp/`, `records/`, `kg/`, `ledgers/`, `cases/`, `incidents/`, `lineage/`, `plugins/`, or
`scripts/` directory exists yet. Founder ratification of repo name/license/TH-EN direction
(`FOUNDATION_v0.2.md` §11) must happen before P-A bootstrap creates any of this for real.

## Open (not resolved by this spec — see `FOUNDATION_v0.2.md` §10/§11)

- The exact section/format content of `venue_track: thai_tci` (which sections it adds to each of
  the 9 genres, beyond the general requirement list in `FOUNDATION_v0.2.md` §6.2) is not yet
  written — stays generic `Dr` until a Thai TCI journal is named (`FOUNDATION_v0.2.md` §11 item
  16, unchanged from v0.1).
