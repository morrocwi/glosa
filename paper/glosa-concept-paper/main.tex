...
% Engine note (honest edge, tier finite_diagnostic): this build system has no
% Unicode-Thai-capable LaTeX engine available -- no luaotfload package, no
% xelatex format, and no root access to install either (checked this
% session: `kpsewhich luaotfload.sty` empty, `fmtutil --byfmt xelatex` finds
% no such format). pdfLaTeX/mathptmx cannot set Thai glyphs directly.
% Work-around: the Blackbox Note's four longer verbatim Thai passages quoted
% in the body are rendered to PNG images (thai_imgs/*.png, generated
% byte-for-byte from the same source strings as
% blackbox/BLACKBOX_NOTE_glosa-paper_2026-09-04.md and
% blackbox/log/entries.jsonl -- not edited or transliterated) and included
% via \includegraphics; every other Blackbox Note reference in this document
% is given as an English gloss with a pointer to the verbatim Thai in the
% source file, rather than reproduced a second time as raw Unicode this
...
% ---- Thin blockquote-style left rule for the verbatim Thai passage images
...
\twocolumn[
\begin{center}
{\LARGE\bfseries\color{TitleBlue} Rigour Without Infrastructure}\\[0.35em]
{\large\color{TitleBlue} Three Propositions on Claim-Card Discipline as a Substitute for Institutional Certification}\\[0.9em]
{\normalsize Yaoharee Lahtee}\\[0.15em]
{\small Open Civil Science Initiative, Thailand}\\[0.1em]
...
\fcolorbox{MetaRule}{MetaBg}{%
  ...
  \textbullet\ \textbf{Licence} CC BY 4.0\\[0.25em]
...
\noindent\colorbox{BriefBg}{%
  ...
  ladder can make a standalone scholar's claim reader-checkable without an institutional
  ...
  self-application demonstration (\Cref{sec:demonstration}). \textbf{Status.} All three
...
independence ladder (I0 self-check through I5 outside human review) before public release --- can
carry the checkable property institutions have historically supplied, without requiring the
institution itself (H1, design; H2, conceptual). A third, narrower proposition (H3, empirical)
holds that cross-vendor AI checking measurably outperforms same-vendor re-checking at catching
undisclosed AI-fill; it is stated but not tested here. Each proposition is placed against a
literature conversation built from 48 independently verified citation cards, and the whole method
is applied reflexively to this paper's own construction, which surfaced real failures --- a
validator warning, a manifest-detected search-mode gap, and a documented citation-verification
correction --- reported as findings, not smoothed into a clean success story. Every claim here sits
at tier \DrTier{} and knowledge state K0; none has passed an independent (I5) human check.
\end{quote}
\vspace{0.7em}
]
...
``Observation'' is abstract; it is a verb, not a starting node. Founder's own line, verbatim
(Blackbox Log \texttt{BBL-2026-09-04-083}, concept DOI \texttt{10.5281/zenodo.22302518}):

\begin{quote}\small
\begin{leftbar}
\includegraphics[width=\linewidth]{thai_imgs/bbl083.png}\\
...
has general-purpose AI assistance. The first human readout of that state, verbatim (Thai, source of
truth), Blackbox Note line 1
(\texttt{blackbox/BLACKBOX\_NOTE\_glosa-paper\_2026-09-04.md\#L1}):

\begin{quote}\small
\begin{leftbar}
...\includegraphics[width=0.85\linewidth]{thai_imgs/l1.png}...\\
...
Founder's line, verbatim, first stated (\texttt{BBL-2026-09-04-086}) and then self-corrected
(\texttt{BBL-2026-09-04-088}, the correction shown here is the one adopted):

\begin{quote}\small
\begin{leftbar}
\includegraphics[width=\linewidth]{thai_imgs/ownership.png}\\
...
(Claude, ``Fable'' seat) drafted candidate hypotheses H1, H2, and H3
...
\midrule
Q1 & \raisebox{-0.2em}{\includegraphics[height=0.9em]{thai_imgs/q1.png}} --- what was actually seen & presence only \\
Q2 & \raisebox{-0.2em}{\includegraphics[height=0.9em]{thai_imgs/q2.png}} --- what the data separates & presence only \\
Q3 & AI \raisebox{-0.2em}{\includegraphics[height=0.9em]{thai_imgs/q3.png}} --- what AI filled in & presence only \\
Q4 & \raisebox{-0.2em}{\includegraphics[height=0.9em]{thai_imgs/q4.png}} --- what is assumed & presence only \\
Q5 & \raisebox{-0.2em}{\includegraphics[height=0.9em]{thai_imgs/q5a.png}}/\raisebox{-0.2em}{\includegraphics[height=0.9em]{thai_imgs/q5b.png}} --- what independent evidence/objection & \textbf{the one genuinely enforceable check} \\
\bottomrule
\end{tabularx}
...
public release, then a person with no institutional affiliation can co-produce knowledge with AI
whose rigour a reader can check for themselves, without an institutional credential serving as the
certifying step.
...
showing the mechanical check does not track the epistemic property it claims to track.
...
\textbf{Statement.} Rigour is not a property institutional machinery confers on a claim from the
outside; it is a property a single claim can carry in its own recorded structure --- institutions
are \emph{one} historical mechanism that has produced this property, not the only possible one.
...
\textbf{Statement.} When claim cards are checked by a different AI vendor from the one that
produced them (I3) instead of the same vendor re-checking (I0/I1), the rate at which undisclosed
AI-fill (a false Q3 field) or undisclosed assumptions (a false Q4 field) is caught rises measurably.
...
relation above independence class I3, and none of the underlying claim cards has yet passed
founder sign-off (\texttt{independent\_check.status: NONE} on all three,
...
(a decorrelated cross-vendor AI route, \texttt{route:codex}, I3) hold. \textbf{Relation column}
...
\midrule
\cite{cite-cstp-boundary-work-001} & Mayes 2022, Citizen Science and Scientific Authority & different & Directly: even a perfectly-documented claim card may still not be treated as legitimate if the surrounding discourse continues to grant authority by credential. \\
\cite{cite-disclosure-not-documentation-001} & Cwik 2026, Disclosure is not documentation & same & Separates disclosure-as-compliance from documentation-as-verifiable-record --- the same separation H1 makes at Q3. \\
...
\cite{cite-rsos-citizen-epistemology-001} & Jaeger et al.\ 2023, An epistemology for democratic citizen science & same & Locates robustness in diversity of standpoint/process --- could argue H1 over-individualizes rigour to one standalone scholar. \\
...
\bottomrule
\end{tabularx}
\caption{H1 (design proposition) against 15 verified citation cards, 5 of them own-lineage.
...
\cite{cite-own-readout-condition-h2-002} & The Readout Condition (Lahtee) & own lineage, not independent & A claim satisfying E-A-D does not need an institution to vouch for it; does not itself argue institutions add nothing. \\
...
cards, 6 own-lineage. Diversity audit (\texttt{h2/litreview\_manifest.yaml}): 3 Thai / 13 English,
...
\textbf{Citation card} & \textbf{Source} & \textbf{Relation} & \textbf{Bearing} \\
\midrule
\cite{cite-selfpreference-judge-001} & Wataoka, Takahashi \& Ri 2024/25, Self-Preference Bias in LLM-as-a-Judge & cited & Bias tracks perplexity/familiarity, not narrowly vendor identity --- the effective variable may not always track vendor boundaries. \\
...
\cite{cite-toohconsistent-003} & Tan et al.\ 2025, Too Consistent to Detect (EMNLP) & same & Same-model repeated sampling cannot catch ``self-consistent'' errors --- structurally the concern H3 raises about same-vendor re-check. \\
...
\cite{cite-mindstudio-crossvendor-010} & MindStudio blog 2026, Cross-Vendor AI Agent Review & same & States H3's own mechanism in practitioner language, but with no measurement, sample, or method of its own --- flagged as concentration risk, not equivalent-weight evidence. \\
...
locator taken from the opened source. The founder's ruling in direct response, verbatim
(\texttt{BBL-2026-09-04-100}):
\begin{quote}\small
\begin{leftbar}
\includegraphics[width=\linewidth]{thai_imgs/bbl100.png}
\end{leftbar}
\end{quote}
\noindent --- ``should we make it a hard rule: if a source is used it must carry a page and line locator plus
a full link, never written from memory --- that reduces the problem at the source, before the
check.'' Adopted immediately (\texttt{BBL-2026-09-04-101}, Thai verbatim in \texttt{entries.jsonl}; gloss: ``update it into the
...
\texttt{independent\_check.status: NONE} --- the founder has ruled on genre and selection at the
project level but has not yet signed off on any specific card instance
(\Cref{sec:claim-matrix}).

\section{AI-assistance disclosure}
\label{sec:ai-disclosure}
This paper is a human--AI co-production, not AI-only automation and not human-only authorship.
Generative AI (Claude, Anthropic, ``Fable'' orchestration seat) assisted with literature retrieval
...
\midrule
C1 & H1: claim-card + E-A-D + independence ladder lets a standalone scholar's claim be reader-checkable without institutional credential. & 15 verified citation cards (\Cref{tab:h1-lit}); \texttt{GLOSA-CC-20260904-0201} & \DrTier{} & joint (data$\to$inference); human (inference$\to$claim) & human \\
C2 & H2: rigour is a portable claim-level property, not conferred solely by institutional machinery. & 16 verified citation cards (\Cref{tab:h2-lit}); \texttt{GLOSA-CC-20260904-0202} & \DrTier{} & joint (data$\to$inference); human (inference$\to$claim) & human \\
C3 & H3: cross-vendor (I3) checking measurably outperforms same-vendor (I0/I1) checking at catching undisclosed AI-fill. \textbf{Untested.} & 17 verified citation cards (\Cref{tab:h3-lit}), all analogical, none a direct test of Q3/Q4 detection; \texttt{GLOSA-CC-20260904-0203} & \DrTier{} & joint (data$\to$inference); human (inference$\to$claim) & human \\
\bottomrule
\end{tabularx}
\caption{Claim Matrix. All three cards: \texttt{k\_state: K0}, \texttt{status: Draft},
\texttt{independent\_check.status: NONE}, \texttt{independence\_class: I3} (the citation-verification
route, not the card's own sign-off).}
...
\centering
\scriptsize
\begin{tabularx}{\linewidth}{@{}p{0.1\linewidth} p{0.07\linewidth} X p{0.15\linewidth} p{0.13\linewidth}@{}}
\toprule
\textbf{ID} & \textbf{Kind} & \textbf{Thai verbatim} & \textbf{Distinction introduced} & \textbf{Became} \\
\midrule
BBL-083 & proposal & see \Cref{sec:problem} image above & Separates ``observation'' (abstract verb) from ``problem state'' (concrete pre-existing thing) & FOUNDATION \S2.1a (\Cref{sec:problem}) \\
BBL-086/088 & finding / correction & see \Cref{sec:standpoint-and-ownership} image above & Which spine nodes must stay human: problem, question, and the \emph{selection} of the hypothesis (088 corrects 086) & FOUNDATION \S2.1c \\
BBL-089 & ruling & \texttt{entries.jsonl} (not reproduced here; column-width) & Ratifies the finer-grained spine diagram (AI-drafted wording, human-ratified agreement) & FOUNDATION \S2.1a diagram \\
BBL-091 & instruction & \texttt{entries.jsonl} (not reproduced here) & Install and use the skill to build this article in full & task B1/B2 launch \\
BBL-095 & ruling & \texttt{entries.jsonl} (not reproduced here) & Selects all three hypotheses (H1, H2, H3) together & \texttt{hypothesis\_selection.yaml} \\
BBL-097 & ruling & \texttt{entries.jsonl} (not reproduced here) & Sets genre to CONCEPTUAL, overriding the router's \texttt{systematic\_review} output & \texttt{genre\_decision.yaml} (\Cref{sec:genre}) \\
BBL-098 & instruction & \texttt{entries.jsonl} (not reproduced here) & Requires colliding own prior papers against world literature, not silence & own-lineage rows, \Cref{sec:literature} \\
BBL-099 & ruling & \texttt{entries.jsonl} (not reproduced here) & English only, no Thai edition; arXiv two-column theme, polished & this document's language and theme \\
BBL-100 & ruling & see \Cref{sec:demonstration} image above & Mandates locator+link, source-first citation, after I3 caught composite quotes & rule 17, \S7.8; \Cref{sec:demonstration} \\
BBL-101 & instruction & \texttt{entries.jsonl} (not reproduced here) & Adopt the citation rule into the system immediately & rule 17 adoption \\
\bottomrule
\end{tabularx}
\caption{Blackbox Note --- curated verbatim lines relevant to this paper's own construction (full
Thai retained; English gloss given inline in the surrounding text per section, not translated here
in place).}
...
\midrule
lens\_in & ai-anthropic/claude-fable & L1 & \texttt{lens\_translation.md} & Drafted \texttt{question\_readout}, \texttt{local\_contrast\_space\_X}, \texttt{access\_relation\_R}, \texttt{claim\_function\_Phi\_z0} from L1 under the Readout Universe lens; not yet independently reviewed. \\
lens\_out & ai-anthropic/claude-fable & L1--L13 & \texttt{hypotheses.md} & Drafted H1/H2/H3 as candidate world-language hypotheses; selection reserved to and performed by the founder (BBL-095). \\
lit\_review & ai-anthropic/claude-fable & H1/H2/H3 & 48 citation cards, 3 \texttt{litreview\_manifest.yaml} & Ran targeted/scoping searches per hypothesis (not systematic), froze manifests, computed diversity/accuracy audits. \\
claim\_card & ai-anthropic/claude-fable & hypotheses + literature & \texttt{GLOSA-CC-20260904-0201/0202/0203} & Filled full-shape cards; validated \texttt{PASS\_WITH\_LIMITS}; not yet founder-signed. \\
paper\_build & ai-anthropic/claude-fable & all of the above & this document & Drafted the concept paper's prose, tables, and \LaTeX{} build; disclosed throughout, never authored. \\
\bottomrule
\end{tabularx}
...
% does not hyphenate URLs on its own); url package below loads with
