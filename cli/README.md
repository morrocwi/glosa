...
|---|---|---|
| `glosa intake new --project P --human-owner H [--origin-blackbox-ref R] [--out-dir D]` | Scaffold a fresh `problem_card` (S1 intake gate). Default `--out-dir`: `records/problems/`. Now also scaffolds `intake.precommitted_resistance_route` (schema.entry-resistance-precommit-field, non-mandatory) and `intake.entry_anchor` (HU-2's full H0 block: `unresolved`, `existing_evidence`, `change_condition`, `verification_intent`, `resistance_route` — all optional, human-authored, never AI-backfilled). | 0 always (scaffold is self-checked, never refuses) |
| `glosa intake validate <path>` | `kernel._schema_validate` against `problem_card.schema.json`. | 0 PASS, 1 FAIL |
| `glosa session open --session-id ID --project P --human-owner H [--entry-anchor REF] [--carried-from REF] [--out-dir D]` | Scaffold a fresh `blackbox_note` carrying an OPEN `session` block (SA-1, SESSION_ARCH_v0.4_SPEC.md §2.1/§2.2/§8) — `session_id`, `session_boundary.opened_at`, `ai_state_at_boundary` (`reset` by default, `carried` only when `--carried-from` names a prior note's `human_retained_residue_ref`), `entry_anchor`. The session-boundary + AI-reset fact lives on the Blackbox Note only (one-fact-one-home) — this is **not** a new `session.yaml` file. Default `--out-dir`: `records/sessions/<session_id>/`. | 0 always |
| `glosa session close --path <note> --retention-note "..." [--retained-direction-ref REF]` | **Fail-closed**: refuses (no file written) unless `--retention-note` is a non-empty, human-authored string — this field is never AI-filled (`'none'` is a legal explicit value). On success, sets `session.session_boundary.closed_at` + `ai_state_at_boundary: reset` and writes the note back in place. Refuses if the note has no `session` block, or the session is already closed. | 0 closed, 1 refused |
...
| `glosa claim validate <path> [--citation-cards GLOB/PATH...]` | `kernel.validate_claim_card` (all §3.3 kernel gate rules + schema). `--citation-cards` loads citation_card files (yaml/json glob patterns and/or literal paths; `*.i3.json`/`*.i5.json` sidecars skipped) and passes them as `citation_cards=[...]`, which is what actually runs D-LENS-UNCITED and rule 28 (inflated bearing) instead of reporting them "not checked". | 0 PASS, 1 FAIL |
...
| `glosa release-gate <manifest> [--cards P...] [--reviews P...] [--citation-cards GLOB/PATH...]` | `kernel.gate_release(manifest, cards, reviews, citation_cards=[...])`. `--citation-cards` loads citation_card files (yaml/json glob patterns and/or literal paths; `*.i3.json`/`*.i5.json` sidecars skipped). **`manifest.status == "Rollback"` short-circuits to ROLLBACK without running the gate** (a human rollback decision overrides gate computation -- `gate_release` itself never returns "ROLLBACK", so this CLI-level check is what actually produces exit 3). | **0** PASS/PASS_WITH_LIMITS, **1** FAIL, **2** HUMAN_REVIEW, **3** ROLLBACK |
...
| `glosa ces check <path> [<path> ...]` | `kernel.ces_check_text`/`kernel.validate_core_epistemic_structure` (methodology/P20_core_epistemic_structure.md, founder ruling 2026-09-07): checks each path for the mandatory Core Epistemic Structure block. `.md`/`.tex`/`.txt` are scanned as raw source text for the three labelled lines plus the non-collapse line (Markdown `**Label:**` and LaTeX `\textbf{Label:}`/`\item[Label]` both match); `.json`/`.yaml`/`.yml` are parsed and checked either as a `core_epistemic_structure` object directly or via that key on a larger document (e.g. a `release_manifest`). Delegates to `scripts/check_core_epistemic_structure.py`'s `check_paths()` -- same implementation, not a re-derivation. | 0 = every path PASSED, 1 = at least one path FAILED or could not be checked |
| `glosa ret check <case.json\|case.csv> [--json]` | RET-Check v0.2 (`methodology/P21_ret_check.md`; `cases/ret/PREREGISTRATION_v0_1.md`): AI-independent (stdlib-only, no network, no LLM) recursive provenance auditor. Reads provenance rows `{claim, agent, parent, source_root, root_independent, record_type, evidence_ref}` grouped by `claim`, computes N_A (distinct endorsing agents), N_P (distinct provenance roots, regardless of declared independence) and N_P^ind (the subset of those roots declared independent -- RET RISK is computed from N_P^ind, never the raw N_P), a recursive-cycle detector over `parent -> agent` edges, an external-interruption check (a `world_record`/independent `review` whose lineage does not pass through a detected cycle), and an optional effective-alternatives regime, then prints a "RET SELF-AUDIT" report with a RET RISK label (LOW/MEDIUM/HIGH) by the pre-registered rule. Delegates to `scripts/ret_check.py`'s `run()` -- same implementation, not a re-derivation; that script is also directly runnable (`python3 scripts/ret_check.py <case> [--json]`, plain-text report by default). This is a structural auditor of a DECLARED graph, never a truth-check on the claim -- see the script's own module docstring and the preregistration's "What the program can NOT verify". | 0 = a report was produced (regardless of the RET RISK label), 2 = the input file was invalid |
| `glosa repro new --id ID --claim "..." [--toledo-codes C1,C2] [--pending-toledo-code] --oracle-kind K --oracle-source "..." [--doi-or-url U] [--version V] --statement "..." --tolerance "..." --human-owner H [--out-dir D]` | Reproduction Ledger (`methodology/P22_reproduction_ledger.md`): scaffold a fresh `reproduction_card` (ALWAYS `cases/repro/<ID>.json`, never `.yaml` -- see the schema/design's own reasons). Writes `preregistered_prediction`/`run: null`/`result.status: PENDING` together, in the same act. **Fail-closed:** refuses (no file written) if `--statement`/`--tolerance` is empty, or if `--toledo-codes` is empty without `--pending-toledo-code`. | 0 written, 1 refused |
| `glosa repro run <path> [--command "cmd"] [--input F...] [--output F...] [--reference V]` | Executes `--command` via `subprocess`, SHA-256-hashes every declared `--input`/`--output` file, compares the run's own observed output against `preregistered_prediction.tolerance` (percent / exact / categorical grammars; `--reference` supplies the external oracle's own value -- no network fetch is implemented), and writes `run{}`/`result{}` back to the card. **Refuses** if `run{}` is already filled (immutable -- use `repro verify`), if `preregistered_prediction.declared_at` is missing, or if `--command` was not given. Delegates to `scripts/repro_check.py`. | 0 a result was written (any status, incl. FAIL/ERROR), 1 refused before executing, 2 the command itself could not be launched |
| `glosa repro verify <path> [--reviewer-identity ID] [--independence-class Ix] [--input F...] [--output F...] [--out-dir D]` | Maker != checker (P10 NC-28/29): a DIFFERENT identity re-executes `run.command` fresh in a clean temp directory (pass the SAME `--input`/`--output` given to `repro run`) and confirms the recomputed hashes match; scaffolds a `review_report.yaml` (`role: SourceAuditor`) either way, never mutating the card. **Refuses** if `--reviewer-identity` equals the card's own `lineage.run_by`. | 0 hashes matched, 1 hashes mismatched or refused, 2 the command could not be re-launched |
| `glosa repro to-ret <path> --claim ID [--review-report P] [--out F]` | Converts one `reproduction_card` into RET-Check provenance rows (`design/RESISTANCE_LADDER_v0_1.md` §4), appending to `--out` (default `cases/ret/<slug>_from_repro.json`). **Refuses** for `oracle.kind == "coq_kernel"` (machine-side resistance, R2 -- never a RET-Check row) and for `human_review` below independence I2 (pass `--review-report` naming a `review_report.yaml`). | 0 rows appended, 1 refused |
| `glosa score <artifact> [--kind toledo-code\|claim_card\|release_manifest] [--toledo-registry P] [--repro-dir D] [--reviews-dir D]` | Resistance Ladder (`methodology/P23_resistance_ladder.md`): prints the R0-R6 rung table for one artifact -- `{held, evidence, reason}` per rung, evidence always a pointer to a file that exists, NEVER a single number (there is no `--score`/scalar flag; this is the one thing the command is built to refuse). Scans a Toledo registry (R2), every matching `cases/repro/*.json` card (R1/R3/R4/R6), and every matching `reviews/routes/**/review_report.*` (R5). A disclosed `FAIL` on a matching card still holds R3/R4/R6. | 0 always (a readout, never a release gate) |
...
| `glosa lit table <problem-slug> <hypothesis-ref>` | Re-renders `dialogue_table.md`'s `## Table` section from every `dialogue_table_row`-shaped file in `records/lit/<slug>/<hyp>/rows/*.{yaml,json}` that passes `dialogue_table_row.schema.json` (invalid rows are reported, not silently dropped or silently included). Also renders the `defeater_class` + `legitimate_defeater` columns (`lrs.dialogue-table-claim-type-column`, deliberately not named `claim_type`, which already routes rule16w on `claim_card.yaml`); a row with a real stance (YES/NO) but no `defeater_class`/`legitimate_defeater` is flagged inline `` `INCOMPLETE` `` and listed in the result's `rows_incomplete_defeater` (lint, not a hard error). | 0 always (invalid rows listed in `rows_invalid`, not an error by themselves) |
| `glosa lit select <problem-slug> --decided-by H --reason R [--chosen "h1,h2"\|"all"] [--park "h:reason" ...] [--cooking-log-ref REF] [--session-id ID]` | Scans `records/lit/<slug>/*/litreview_manifest.*` for every hypothesis with a manifest, builds the `candidates` array, and writes `hypothesis_selection.<ext>`. **No `--auto`**: every candidate not in `--chosen` MUST have a matching `--park '<hyp>:<reason>'` or the command refuses (a parked hypothesis's reason is never fabricated). `--session-id` writes SA-2's session-grouping key (from `glosa session open`); `retained_direction` is always scaffolded `unknown` (NC-77 — retention is never direction-evidence on its own; a human/reviewer sets it otherwise only with a linked evidence_relation). | 0 written, 1 refused/no manifests found |
